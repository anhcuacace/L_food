...
class CategoryAdapter(
    var context: Context, var options: FirebaseRecyclerOptions<CategoryItem>, var type: Int
        ...
        if (type == 1) {
            return CategoryViewholder(
                LayoutInflater.from(parent.context)
                    .inflate(R.layout.item_rclv_category, parent, false)
            )
        } else {
            return CategoryViewholder(
                LayoutInflater.from(parent.context)
    ...
    override fun onBindViewHolder(holder: CategoryViewholder, position: Int, model: CategoryItem) {
        ...
        Glide.with(holder.itemimg.context).load(model.img)
